...
/*-------------------------- ADD for support plugin JWT --------------------------*/
// writeLengthEncodedString writes a string in MySQL's Length-Encoded String format.
// This is used in the handshake/auth packet (e.g., for JWT support).
...
func writeLengthEncodedString(buf *bytes.Buffer, b []byte) {
	...
	case length < 0x10000:
		...
		_ = buf.WriteByte(byte(length >> 8))
	case length < 0x1000000:
		buf.WriteByte(0xFD)
		_ = buf.WriteByte(byte(length))
		_ = buf.WriteByte(byte(length >> 8))
		_ = buf.WriteByte(byte(length >> 16))
	default:
		buf.WriteByte(0xFE)
...
func encodeAttributes(attrs map[string]string) []byte {
	data := make([]byte, 0)
	for k, v := range attrs {
		data = appendLengthEncodedString(data, k)
...
func (mc *mysqlConn) writeHandshakeResponsePacket(authResp []byte, plugin string) error {
	...
	_ = data[4*3+23] // Check buffer limit

	// Client capabilities [32 bits]
	binary.LittleEndian.PutUint32(data[4:], uint32(mc.capabilities))

	// MaxPacketSize [32 bits]
	...
	authPlugin := plugin
	if v, ok := mc.cfg.Params["auth_client_plugin"]; ok && v != "" {
		authPlugin = v
	}
	...
	// Add authentication plugin name
